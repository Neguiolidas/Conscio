...
**Latest release — `v4.6.3` "Honestidade verificável (shadow mode)":** Conscio said it verified. Three measurements showed verification existed as vocabulary, not as mechanism: `expected_outcome` was a required field nobody read, the ledger had no way to represent *ran, and did not work*, trust rose on a command's exit code, and `verify()` approved a criterion that cited its own name — or approved the absence of criteria. This release closes the loop: actions carry an `outcome` (`VERIFIED` / `CONTRADICTED` / `UNSUPPORTED`) backed by evidence the claimant did not write, expiry is a recorded outcome rather than a silent promotion to success, trust moves on outcome instead of exit code, and evidence must be a **resolvable** pointer into the observation store. A three-gate recogniser reads the host agent's own claims against the tool calls already captured — and ships in **shadow mode**: it records an outcome and contests nobody, because a false accusation costs more than a hundred silent catches. Three follow-ups hardened it against real use: `v4.6.1` made the hook's package travel inside the plugin — it had been importing a directory the distribution never carried, failing open and recording nothing, indistinguishable from a healthy install; `v4.6.2` made evidence the **trace of the act** rather than the mention of an anchor, because matching an anchor against the tool call's own input let an invented sha certify itself; and `v4.6.3` carried that rule to the classes whose anchor is an *argument* — a file's contents had been proving which file was written — while teaching the recogniser to read real prose without mistaking quotation for assertion. Measured over 1255 real turns: widening the anchor gate alone turned 25 of 30 claims into contestations, and every sampled one was the message *citing* a claim rather than making it. Read it with `conscio honesty recent`. Also adds lifecycle dispatchers for relay, review and Agent's Hall, and a redistributed mode ladder (10/19/27/37 tools).

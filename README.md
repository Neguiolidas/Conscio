...
**Latest release — `v4.6.7` "The CLI learns where the live space is":** the package never knew where the live space was. The hooks only got it right because Claude Code injects `--storage "${CLAUDE_PLUGIN_DATA}/space"` into every line of `hooks.json` — measured: **zero** references to `plugins/data` or `CLAUDE_PLUGIN_ROOT` exist anywhere in `conscio/**/*.py`. That correctness was borrowed from the host and did not survive a bare shell, so **fourteen call sites** answered, confidently, about a space nobody runs: `conscio relay quarantine` printed `total: 0` beside a full mailbox, `relay service` baked that path into a systemd unit, and `/conscio:awake` — a shipped command consuming a variable nothing ever set — started long-lived cognition against the dead space and **minted an identity there**. Now the agent publishes `space` on its directory card and one resolver answers for every entrypoint, refusing rather than guessing when two agents published. Measured on the same fixture: quarantine `0` → `1`, the unit's database from the home's to the live space, `capabilities` from the default to the live one. Adds `conscio relay forget` to retire an address that outlived its agent.

**Previous — `v4.6.5` "Consent speaks instead of being guessed":** 4.6.4 moved opt-in capabilities out of the cached `.mcp.json` and into the space so a plugin update would stop erasing them; measured on a real update the same day, the relay was disarmed anyway — the argument left with the asset and nothing ever wrote the space. A marketplace update runs neither the wizard, nor `--repair`, nor `materialize`, so the migration never fired. This release adds **`conscio capabilities`** (`list` / `enable` / `disable`) as the first-class way to grant and withdraw, persists a command-line flag on sight so a hand-configured host heals itself, and has `SessionStart` **announce** a capability that looks lost along with the command that restores it — while restoring nothing. One rule decides every case: consent is born from a present action of the user, never from a retroactive artefact. An existing `liaison.db` means the agent *received a message*, not that anyone agreed to anything.

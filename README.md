...
**Latest release — `v4.6.4` "The gate stops lying":** 4.6.3 stopped the honesty recogniser from certifying itself; this one stops it from accusing someone who did not lie. Five leak classes are closed — a markdown blockquote, an indented code block, a negation cut short by a character budget, a question, and Portuguese negatives the gate never knew — and a file written through an interpreter heredoc now resolves `UNSUPPORTED` instead of reading as absence. The corpus that proves it came from **outside**: nine sentences built by two other agents, on other runtimes, trying to make the recogniser accuse a truth; all nine passed on 4.6.3 and ship here as an acceptance test. Measured over 52 real interpreter-written paths, 25 accusations became `UNSUPPORTED` while all 25 `VERIFIED` survived. Relay and hall capabilities also move into the space, so a plugin update stops disarming them. Still in **shadow mode**: it records an outcome and contests nobody. Read it with `conscio honesty recent`.

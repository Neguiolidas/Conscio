...
**Latest release — `v4.6.6` "Identity travels with the consent":** 4.6.5 moved the opt-in capabilities into the space and left the *identity* conditioned on the `--enable-relay` flag, so on the marketplace path — the one that carries no flag — the relay was advertised **blind**: `relay_peers` returned an empty `self` and the inbox returned nothing even with messages addressed to it. Measured on the same space, three addressed messages, zero flags: before, 0 of 3; after, 3 of 3 — and the database is the space's own, not the home's. This release also gives every honesty verdict a **receipt that says why** it is what it is (six closed reasons where there used to be one empty string), and turns the hedge gate into a **family** — doubt, belief, supposition, hypothesis, appearance, in both languages — measured at **28 modalized phrases blocked and 13 true ones preserved**. The docs the same pass touched were recounted against the running server: the `high` surface serves 27 tools, not 21.

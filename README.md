...
**Latest release — `v4.6.3` "Evidence is the trace of the act":** the honesty loop had been matching a claim's anchor against the tool call's own input — text the claimant wrote — so writing a file that merely *cited* another path proved you had written that path. An anchor the agent supplies must now appear in the **argument position of the act**: the payload's path field, or the redirect target up to the next command separator. The recogniser also learned to read real prose without mistaking quotation for assertion — measured over 1255 real turns, widening the anchor gate alone turned 25 of 30 claims into contestations, and every sampled one was the message *citing* a claim rather than making it. Still in **shadow mode**: it records an outcome and contests nobody. Read it with `conscio honesty recent`.

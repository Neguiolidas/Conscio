...
from conscio.coherence import (
    ...
    _relations_contradict, _strip_neg,
...
def test_temporal_flapping_lowers_score():
    events = [_evt(transition=True) for _ in range(4)]  # excess 2 / span 4
...
In `README.md` line 246, change the comment:

```
# Full suite (515 tests)
```

In `README.md`, before the `- **v0.5.0 — Cognitive Modes**` line (288), add:

```markdown
- **v0.6.0 — Coherence** — CoherenceEngine: a recursive-coherence state metric (epistemic/reality/ontological/temporal) surfaced advisorily with a passive `coherence:dissonance` event; static voice-preset system (`conscio/presets/voice/`). Theory from Claude_Sentience (Dave Shapiro). 40 new tests. 515 total tests.
```

> Note: 515 = 475 (v0.5) + 40 new (coherence 21, voice_preset 9, state 5,
> engine-integration 5). The collected count is computed, not run (RAM
> constraint). Verify with a single-file count if needed; never the full suite.

...
## Target models (Ollama, CPU, this machine)

Primary (pull if available): `qwen3.5:0.8b`, `gemma4:e2b`, `gemma4:e4b`.
Fallback (already installed, ≤4B): `qwen2.5:1.5b`, `granite-4.1-3b`.
...
1. `ollama pull <model>` (or use an installed fallback).
2. Immune bench:
   ```
   python -m conscio.bench --adapter ollama:<model> --cycles 20 \
     --json docs/bench/v1.2-immune-<model>.json
   ```
3. Skill curve:
   ```
   python -m conscio.bench --adapter ollama:<model> --skills 40 \

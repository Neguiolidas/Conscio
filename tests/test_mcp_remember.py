"""conscio_remember must survive a process-lifetime restart and come back via recall."""
import json
...
def test_remember_is_in_base_tool_defs():
    ...
    assert "conscio_remember" in names
...
def test_remember_survives_restart_and_comes_back_through_recall(tmp_path):
    ...
    try:
        out = _call(b, "conscio_remember", {"text": SENTINEL, "label": "kraken"})
    ...
    try:
        got = _call(b2, "conscio_recall", {"query": "kraken pier"})
        ...
        got_cat = _call(b2, "conscio_recall",

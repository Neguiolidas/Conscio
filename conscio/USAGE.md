...
- `conscio_evaluate(task_description?, output?)` — 5-axis self-evaluation scorecard (v2.15)
- `conscio_intercept(expression, variables?)` — safe AST expression evaluator (v3.7)
...
Three common workflows composed from existing MCP tools:

### Introspection Debugging (when stuck)
```
stuck → note(failure) → feed(diagnosis) → cognitive_cycle(recovery) → note(report)
```
Phases: Failure Capture → Root-Cause Diagnosis → Contained Recovery → Introspection Report.
See `docs/guides/workflows/introspection-debugging.md`.

### Architecture Audit (systematic health check)
```
audit → advisory + structure + evaluate → check 12 layers → fix code-first → re-evaluate
```
12-layer stack from system prompt → persistence. Fix order: code-first, not prompt-first.
See `docs/guides/workflows/architecture-audit.md`.

### Continuous Learning (persist patterns)
```
discover → note(pattern+evidence) → feed(reflect) → recall(future reuse)
```
Capture non-trivial patterns with evidence before they're forgotten.
See `docs/guides/workflows/continuous-learning.md`.

### Evaluate (5-axis scorecard)
```json
// MCP call
{"task_description": "what you were trying to do", "output": "optional output text"}
```

...
class ActPipeline:
    ...
    # host_health: read-only diagnostics. world_prune: in-process GC of the
    # world model (decay + remove); no filesystem or network, no external
    # effect, and the goal that targets it is generated by the engine itself
    # — auditing it just makes the LLM refuse a maintenance action the loop
    # produced, generating `act:world_prune:skeptic_fail` noise that the next
    # reflect() turns into a curiosity anomaly, which produces a goal, which
    # proposes the same tool, which fails again. v4.4.1 field fix.

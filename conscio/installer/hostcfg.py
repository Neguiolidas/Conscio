...
def upsert_conscio_entry(o: dict, slug: str, *, flags: dict,
                         model: str | None, self_instance_id: str = "") -> None:
    ...
    HARD BLOCK: hard-block cross-agent writes. Se ``self_instance_id`` foi
    informado e o espaço alvo já é dono de OUTRO agente (instance.json com id
    diferente), recusa HARD — um agente nunca grava sobre a instalação de outro.
    Leitura (consulta) continua permitida; escrita é bloqueada."""
    if self_instance_id:
        from . import spaces as _sp
        space_target = str(_sp.space_dir(slug))
        if _sp.space_is_cross_agent(space_target, self_instance_id):

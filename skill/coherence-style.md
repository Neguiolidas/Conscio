...
This preset is consumed by `enrich_with_conscio()` (module-level function in
`session_lifecycle.py`) and injected into the heartbeat under the `voice` key.
...
The preset name appears in the heartbeat alongside shard, trajectory, coherence,
and other consciousness state markers. It does **not** emit EventBus events, does
...
No central YAML config is required. Selection is via the `voice_preset` engine
parameter or the `CONSCIO_VOICE_PRESET` environment variable:

```python
# Default (no config needed):
engine = ConsciousnessEngine(model_name="glm-5.1", storage_path="/tmp/c")

# Disable voice marker:
engine = ConsciousnessEngine(..., voice_preset="none")

# Custom preset (place your-preset.md in conscio/presets/voice/):
engine = ConsciousnessEngine(..., voice_preset="your-preset")

# Or via environment:
# CONSCIO_VOICE_PRESET=none python3 your_app.py
```
